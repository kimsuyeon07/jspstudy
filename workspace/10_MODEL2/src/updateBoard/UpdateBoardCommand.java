...
public class UpdateBoardCommand implements BoardCommand {
	...
	public ModelAndView execute(HttpServletRequest request, HttpServletResponse response) {
		...
		// 첨부파일이 있는 폼은 MultipartRequest로 처리한다.
		...
		try {
			...
			int result = BoardDAO.getInstace().updateBoard(dto);
			if (result == 0) {
				...
				mav = new ModelAndView("/10_MODEL2/selectOneBoard.b?idx=" + multipartRequest.getParameter("idx"), true);  // 반드시, redirect
			}
