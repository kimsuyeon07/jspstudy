...
public class Ex05 extends HttpServlet {
	...
	/*
		...
		// Optional : 내용이 들어 있지 않으면 Exception을 반환
		// Optional of()는 null이 '아닌' 인수를 처리한다.
		System.out.println(opt1.get());
		// get() 메소드는 Optional에 저장된 인수를 꺼낸다.
	*/
	/*
		String name = request.getParameter("name");
		Optional<String> opt1 = Optional.ofNullable(name); // Optional.ofNullable()은 null인 인수도 처리한다. (모두 처리한다)
		if (opt1.isPresent()) { // isPresent() : 해당 함수에 값이 존재하면 true 반환
			// opt1에 데이터가 존재하면  (null이 아니면)
			System.out.println(opt1.get());
		}
	*/
	/*
