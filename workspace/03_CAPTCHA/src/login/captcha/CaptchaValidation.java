...
public class CaptchaValidation {
	...
	// 1. 반환타입 : boolean
	// 2. 메소드명 : getValidate
	// 3. 매개변수 : HttpServletRequest request (parameter인 user_key + session에서 key값을 빼기 위해서) 
    public static boolean getValidate(HttpServletRequest request) {
        ...
        /* 수정사항 _ request의 session에 저장한 값 꺼내기 + request.getParameter() */
        String key = request.getSession().getAttribute("key").toString() ; // 캡차 키 발급시 받은 키값 / 문자열타입으로 변환( .toString() )
